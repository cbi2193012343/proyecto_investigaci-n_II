...
\large Proyecto de Investigación II }
...
% ============================================================
% Inicio fusionado desde: Poda (6)(2).tex
% ============================================================
...
En el caso finito, toda distribución de probabilidad puede representarse como un punto del símplex. Esta representación aparece de manera natural en la teoría axiomática de la probabilidad \citep{Kolmogorov1933}. A partir de esta observación, el problema del cambio informacional puede reformularse como el estudio de transformaciones internas del símplex compatibles con la interpretación probabilística. En este trabajo se fija un dominio preciso ---estados informacionales discretos inducidos por particiones medibles finitas--- y se introducen transformaciones elementales definidas mediante correcciones relativas. Estas transformaciones dan lugar a operadores bien definidos sobre el símplex y permiten incorporar funcionales informacionales para comparar estados \citep{Csiszar1975}.

El punto de vista adoptado no pretende reemplazar marcos ya establecidos para dinámicas estocásticas o reglas particulares de actualización. Un ejemplo clásico de tales marcos lo proporcionan las cadenas de Markov. También existen formulaciones variacionales para la evolución de medidas en espacios métricos \citep{JordanKinderlehrerOtto1998}. Más modestamente, el objetivo aquí es aislar un formalismo matemático en el que las transformaciones entre distribuciones finitas puedan estudiarse directamente como objeto de análisis. En esta formulación, los cambios entre estados se modelan mediante correcciones relativas que preservan la masa total. En el desarrollo posterior, estas correcciones admitirán interpretación composicional \citep{Aitchison1986}. También admitirán una interpretación por cambio de medida en el caso discreto. Cuando sea pertinente, se indicará además su interpretación geométrica local en términos de la métrica de Fisher \citep{Amari2016}.

El interés de este problema no es únicamente formal. En diversas situaciones aparecen familias finitas de probabilidades o vectores normalizados cuya evolución contiene información relevante sobre estabilidad, convergencia o respuesta al entorno. En particular, las actualizaciones multiplicativas sobre el símplex aparecen en optimización y aprendizaje \citep{KivinenWarmuth1997}. Por otra parte, la estructura geométrica del símplex ha sido estudiada sistemáticamente en análisis composicional \citep{Aitchison1986}. También ha sido analizada desde la geometría de la información \citep{AmariNagaoka2000}.
...
\begin{itemize}
    \item \textbf{Análisis composicional (Aitchison).} Proporciona la estructura algebraico--geométrica interna del símplex cuando las transformaciones relevantes son multiplicativas y seguidas de cierre o normalización. En particular, las operaciones de perturbación y potenciación permiten trabajar con cantidades relativas dentro del símplex \citep{Aitchison1986}.

    \item \textbf{Geometría de divergencias (Csiszár).} Aporta funcionales de comparación entre distribuciones. En particular, las $f$-divergencias proporcionan un marco general para medir discrepancias entre estados probabilísticos \citep{Csiszar1975}. Dentro de esta familia, la divergencia de Kullback--Leibler ocupa un lugar central en este trabajo \citep{KullbackLeibler1951}.

    \item \textbf{Geometría de la información (Amari).} Proporciona la métrica de Fisher y el lenguaje diferencial que permite interpretar gradientes y direcciones de cambio en el interior del símplex \citep{AmariNagaoka2000}. Este punto de vista se utilizará sólo cuando sea necesario para interpretar localmente la dinámica \citep{Amari2016}.

    \item \textbf{Formulaciones variacionales en espacios de medidas (Wasserstein/JKO).} Este marco no se desarrolla en el documento como teoría principal. Sin embargo, se toma como referencia cuando se discuten analogías entre funcionales, esquemas discretos y dinámicas de descenso \citep{JordanKinderlehrerOtto1998}. La formulación abstracta de estos flujos en espacios métricos puede consultarse en \citep{AmbrosioGigliSavare2005}.
\end{itemize}

La convención adoptada será la siguiente. Las transformaciones elementales se formulan principalmente en la geometría composicional del símplex \citep{Aitchison1986}. Los funcionales de comparación se expresan mediante divergencias informacionales \citep{Csiszar1975}. Cuando resulte necesario, las interpretaciones diferenciales se harán explícitas en términos de la métrica de Fisher \citep{Amari2016}. De manera análoga, las referencias a esquemas variacionales se entenderán sólo como punto de comparación conceptual \citep{JordanKinderlehrerOtto1998}.
\section{Fundamentos probabilísticos y geométricos del control informacional}
...
\subsection{Probabilidad, particiones y símplex}

Esta subsección fija el dominio probabilístico discreto en el que se desarrollará el trabajo. El punto de partida es un espacio de probabilidad en el sentido de Kolmogórov \citep{Kolmogorov1933}. A partir de una partición medible finita se asocia a la medida un vector de probabilidades, que se interpretará como estado informacional discreto. Dicho vector pertenece al símplex de probabilidad.
...
\begin{proposicion}[Estado inducido por una partición finita]\label{prop:estado_simplex}
...
\subsection{Correcciones locales y cambio de medida}
...
\begin{proposicion}[Cambio de medida discreto inducido por una corrección local]
\label{prop:QYprobabilidad}
Sea $(\Omega,\mathcal F,P)$ un espacio de probabilidad y sea
$\Pi=\{E_1,\dots,E_n\}$ una partición medible finita. Sea
$X\in\Delta_n$ el estado inducido por $\Pi$, es decir,
$X_i=P(E_i)$. Si $Y\in C(X)$, entonces existe una única probabilidad
$Q_Y^\Pi$ sobre la álgebra finita generada por $\Pi$ tal que
\[
Q_Y^\Pi(E_i)=P(E_i)(1+Y_i),
\qquad i=1,\dots,n.
\]
\end{proposicion}

\begin{proof}
Construimos $Q_Y^\Pi$ sobre $\sigma(\Pi)$ de la siguiente manera. Si
$A\in\sigma(\Pi)$, entonces $A$ es unión finita de celdas de la partición, y definimos
\[
Q_Y^\Pi(A)
=
\sum_{\{i:\,E_i\subset A\}} P(E_i)(1+Y_i).
\]
Como $Y\in C(X)$, se tiene $X_i(1+Y_i)\ge 0$ para todo $i$. Dado que
$X_i=P(E_i)$, todas las masas asignadas por $Q_Y^\Pi$ son no negativas.

Además,
\[
Q_Y^\Pi(\Omega)
=
\sum_{i=1}^n P(E_i)(1+Y_i)
=
\sum_{i=1}^n X_i(1+Y_i)
=
1,
\]
por la condición de admisibilidad de $Y$. Por tanto, $Q_Y^\Pi$ tiene masa total uno.
La aditividad finita se sigue directamente de que las celdas de $\Pi$ son disjuntas.

Finalmente, la unicidad se obtiene porque todo elemento de $\sigma(\Pi)$ es unión
de celdas de $\Pi$. Por tanto, una probabilidad sobre $\sigma(\Pi)$ queda
determinada por sus valores sobre $E_1,\dots,E_n$.
\end{proof}

\begin{corolario}[Compatibilidad del cambio de medida inducido con el operador local]
\label{cor:compatibilidadCambioMedida}
Sea $(\Omega,\mathcal F,P)$ un espacio de probabilidad y sea
$\Pi=\{E_1,\dots,E_n\}$ una partición medible finita. Sea
$X\in\Delta_n$ el estado inducido por $\Pi$, es decir,
$X_i=P(E_i)$. Si $Y\in C(X)$ y $Q_Y^\Pi$ denota la única probabilidad
sobre $\sigma(\Pi)$ determinada por
\[
Q_Y^\Pi(E_i)=P(E_i)(1+Y_i),
\qquad i=1,\dots,n,
\]
entonces
\[
\bigl(Q_Y^\Pi(E_1),\dots,Q_Y^\Pi(E_n)\bigr)
=
T_Y^{\mathrm{loc}}(X).
\]
Equivalentemente, para cada $i=1,\dots,n$,
\[
Q_Y^\Pi(E_i)=T_Y^{\mathrm{loc}}(X)_i,
\]
\end{corolario}

\begin{proof}
Por la definición de las masas de $Q_Y^\Pi$ sobre las celdas de la partición,
\[
Q_Y^\Pi(E_i)=P(E_i)(1+Y_i).
\]
Como $X_i=P(E_i)$, se obtiene
\[
Q_Y^\Pi(E_i)=X_i(1+Y_i).
\]
...
Q_Y^\Pi(E_i)=T_Y^{\mathrm{loc}}(X)_i
\]
para todo $i=1,\dots,n$. Esto prueba la igualdad vectorial.
\end{proof}

\begin{observacion}
El corolario anterior identifica la actualización local con la redistribución de probabilidad inducida por la medida $Q_Y^\Pi$ sobre la partición original. En este sentido, el operador $T_Y^{\mathrm{loc}}$ no actúa sólo como transformación algebraica sobre el símplex, sino como representación discreta de un cambio de medida por sección s.
\end{observacion}
\subsection{Operadores globales multiplicativos y composición}

Esta subsección pasa del mecanismo local de la subsección anterior a una familia de operadores definida de manera uniforme sobre el interior del símplex. Se consideran correcciones multiplicativas positivas y se normaliza el resultado para conservar la suma unitaria. Esto da lugar a operadores globales sobre $\Delta_n^\circ$ y permite describir su composición de forma explícita.

\begin{definicion}[Corrección global y operador multiplicativo normalizado]
\label{def:Cglob}
\label{def:Tglobal}
Una corrección global es un vector $Y\in\mathbb R^n$ tal que
\[
1+Y_i>0,\qquad i=1,\dots,n.
\]
Denotamos por
\[
C_{\mathrm{glob}}
=
\{Y\in\mathbb R^n:\ 1+Y_i>0\ \text{para todo } i\}
\]
al conjunto de correcciones globales. Para cada $Y\in C_{\mathrm{glob}}$, el vector de factores multiplicativos asociado viene dado por
\[
a(Y):=(1+Y_1,\dots,1+Y_n)\in\mathbb{R}_{>0}^n.
\]

Si $Y\in C_{\mathrm{glob}}$, definimos el operador multiplicativo normalizado
$T_Y:\Delta_n\to\Delta_n$ por
\[
T_Y(X)_i
=
\frac{X_i(1+Y_i)}
{\sum_{k=1}^n X_k(1+Y_k)},
\qquad i=1,\dots,n.
\]
El denominador es estrictamente positivo para todo $X\in\Delta_n$, porque
$X$ tiene masa total uno y todos los factores $1+Y_k$ son positivos.
En particular, la fórmula está definida también en la frontera de $\Delta_n$;
sin embargo, la estructura algebraica de estos operadores se estudiará
principalmente sobre $\Delta_n^\circ$.
\end{definicion}
...
\begin{observacion}[Relación con el operador local de la subsección anterior]\label{obs:localVsGlobal}
...
\]
En este sentido, la estructura multiplicativa de los factores positivos se convierte en una estructura aditiva en coordenadas logarítmicas la cual es compatible con el uso de coordenadas relativas en análisis composicional \citep{Aitchison1986}.
\end{observacion}

\subsection{Divergencia KL, métrica de Fisher y dinámica inducida}

En esta subsección se introduce el funcional informacional
\[
E_P(X):=D_{\mathrm{KL}}(P\|X),
\]
se calcula su gradiente respecto de la métrica de Fisher en el interior del símplex y se obtiene la dinámica continua asociada. Finalmente, se muestra que esta dinámica es compatible con la familia de operadores globales introducida en la subsección anterior.

El resultado principal de la subsección es el Teorema~\ref{teo:equivalenciaCD}, donde se establece la compatibilidad estructural entre la dinámica Fisher--KL y la familia de operadores globales de la subsección anterior.
...
\begin{observacion}[Dominio de $E_P$]\label{obs:KLsoporte}
...
u,v\in T_X\Delta_n.
\]
La forma anterior corresponde a la métrica de Fisher--Rao en el caso finito. La caracterización por invariancia de esta métrica se apoya en \citep{Chentsov1982}. El tratamiento geométrico sistemático se desarrolla en \citep{AmariNagaoka2000}. La formulación moderna usada aquí sigue la exposición de \citep{Amari2016}.
\end{definicion}
...
\begin{proof}
La no negatividad corresponde a la formulación de divergencias de \citep{Csiszar1975}. El criterio de igualdad para la divergencia de Kullback--Leibler se usa en la forma estándar expuesta en \citep{CoverThomas1991}. Aplicando ese resultado a
\[
E_P(X)=D_{\mathrm{KL}}(P\|X),
\]
...
\begin{teorema}[Equivalencia estructural entre operadores globales y dinámica Fisher--KL]
\label{teo:equivalenciaCD}
Sea $P\in\Delta_n^\circ$ fijo. Considérese el operador informacional global $T_Y$
de la subsección de operadores globales (Def.~\ref{def:Tglobal}), definido para $Y\in C_{\mathrm{glob}}$
...
\]
Por la Proposición~\ref{prop:bayes_como_TY}, cuando \(\ell\) proviene de una verosimilitud inducida por una observación, \(\ell_i=P(E\mid H_i)\), y \(X\) es el estado informacional previo asociado a \(\Pi_H\), el operador \(T_{Y^\ell}(X)\) coincide con el posterior bayesiano \(P(H_i\mid E)\). Por brevedad, en esta subsección se escribe \(T_\ell:=T_{Y^\ell}\).
\end{definicion}
...
Así, el criterio de Control Informacional no consiste primero en fijar un número \(\alpha_t\), sino en seleccionar la observación que aproxima el posterior al objetivo \(\mathcal P\). El parámetro de avance aparece después, como una medida del efecto que una observación admisible produce sobre la divergencia entre el posterior obtenido y el objetivo bayesiano.
...
\begin{definicion}[Familia observacional admisible y posterior inducido]
Sea
\[
\Pi_H=\{H_1,\dots,H_n\}
\]
...
El \emph{posterior inducido} por \(E\) desde el estado \(X_t\) se define por
\[
X_t^E
:=
T_{\ell_t^{(E)}}(X_t).
...
%====================================================================
\section{Sección 6 -- Semántica markoviana}
%====================================================================
...
\section{Sección 7 -- Control Óptimo Informacional y Defecto de Ordenamiento}
...
Las composiciones \(B\to M\) y \(M\to B\) requieren una base probabilística común. La base markoviana común se toma de la formulación de cadenas de Markov de \citep{Norris1997}. La lectura bayesiana de estabilidad posterior se usa como referencia contextual en \citep{Sprungk2020}. En este marco, el estado inicial no se interpreta sólo como un vector abstracto del símplex, sino como la ley discreta inducida por una variable aleatoria finita.
...
De acuerdo con la Proposición~\ref{prop:bayes_como_TY}, la parte bayesiana actúa como una corrección global del estado. En cambio, de acuerdo con la Proposición~\ref{prop:markov_como_correccion_local}, la parte markoviana admite una representación local alrededor del estado sobre el cual se aplica.
...
El orden Markov--Bayes no usa la observación para corregir la ley inicial. Primero forma la ley predictiva del estado siguiente mediante el operador markoviano. La descripción de una cadena por su ley inicial y su matriz de transición sigue a \citep{Norris1997}. La acción del operador markoviano sobre medidas y su control en norma de variación se relaciona con \citep{GaubertQu2014}.
...
\]
La pregunta ya no es solamente si la recurrencia esta bien definida, sino si su dependencia respecto del estado inicial es controlada. Por ello se busca probar una cota de la forma
\[
|V_\tau(X)-V_\tau(Y)|
\le
L_\tau(m)\|X-Y\|_1
...
D_{\mathrm{KL}}.
\]
El uso de divergencias KL en los costos de etapa es coherente con formulaciones donde el costo mide la modificación de una distribución o de una trayectoria probabilística inducida por una acción. El uso de costos probabilísticos asociados a acciones es coherente con la formulación de control KL de \citep{Todorov2009EfficientComputationOptimalActions}. La interpretación del problema como inferencia sobre trayectorias se apoya en \citep{KappenGomezOpper2012OptimalControlAsAGraphicalModelInferenceProblem}. En este bloque las referencias solo motivan la estructura; las cotas se prueban directamente a partir de las definiciones de la Sección 7.
\end{observacion}
...
La literatura de programación dinámica introduce precisamente esta separación entre decisión presente y subproblema restante \citep{ref18}. En control óptimo, esta estructura se interpreta como una contribución acumulada de trayectoria junto con un costo terminal \citep{Kappen2005LinearTheoryForControlOfNonLinearStochasticSystems}; en control KL, el costo probabilístico se expresa mediante divergencias y el problema se propaga hacia atrás por funciones de valor o mensajes \citep{KappenGomezOpper2012OptimalControlAsAGraphicalModelInferenceProblem}. En la Sección 7, esta lectura se concreta en el hecho de que la regularidad de \(V_{\tau-1}\), junto con las cotas uniformes para \(c\) y \(T^u\), produce regularidad de \(V_\tau\).
